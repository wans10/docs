---
title: "Streaming"
description: "Learn how to implement streaming responses with LLM Hub's API. Complete guide to Server-Sent Events (SSE) and real-time model outputs."
icon: 'bars-staggered'
---

The LLM Hub API allows streaming responses from _any model_. This is useful for building chat interfaces or other applications where the UI should update as the model generates the response.
...
For SSE (Server-Sent Events) streams, LLM Hub occasionally sends comments to prevent connection timeouts. These comments look like:
