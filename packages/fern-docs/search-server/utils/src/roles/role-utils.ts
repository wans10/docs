...
export function flipAndOrToOrAnd(andOrRoles: string[][]): string[][] {
  ...
  const combinations: string[][] = combine(
    andOrRoles.filter((roles) => roles.length > 0)
  );
...
export function isVisibleByEveryone(roles: string[][]): boolean {
  return roles.some(
    (roles) => roles.length === 1 && roles[0] === EVERYONE_ROLE
  );
}
...
export function modifyRolesForEveryone(
  ...
  if (!authed || roles.length === 0 || isVisibleByEveryone(roles)) {
    return {
      roles: [[EVERYONE_ROLE]],
      authed: authed && !isVisibleByEveryone(roles),
    };
  } else {
    return {
      roles: roles.map((roles) =>
        roles.filter((role) => role !== EVERYONE_ROLE)
      ),
      authed: true,
    };
  }
