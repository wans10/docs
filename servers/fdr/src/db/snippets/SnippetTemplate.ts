...
export class SnippetTemplateDaoImpl implements SnippetTemplateDao {
  ...
  public async loadSnippetTemplate({
    ...
    const sdkFromRequest = await getSdkFromSdkRequest(
      this.prisma,
    ...
    if (loadSnippetTemplateRequest.endpointId.identifierOverride != null) {
      snippetTemplate = await this.prisma.snippetTemplate.findFirst({
        where: {
          ...
          identifierOverride:
            loadSnippetTemplateRequest.endpointId.identifierOverride,
    ...
    return {
      ...
      snippetTemplate: {
        ...
        functionInvocation: readBuffer(
          snippetTemplate.functionInvocation
        ) as FdrAPI.Template,
        clientInstantiation: readBuffer(
          snippetTemplate.clientInstantiation
        ) as string,
      },
    };
  }
  ...
  public async storeSnippetTemplate({
    ...
    await this.prisma.$transaction(async (tx) => {
      const snippets: Prisma.Enumerable<
        WithoutQuestionMarks<Prisma.SnippetTemplateCreateManyInput>
      > = [];
      ...
      await Promise.all(
        storeSnippetsInfo.snippets.map(async (snippet) => {
          ...
          snippets.push({
            ...
            functionInvocation: writeBuffer(
              snippet.snippetTemplate.functionInvocation
            ),
            clientInstantiation: writeBuffer(
              snippet.snippetTemplate.clientInstantiation
            ),
          });
  ...
  public DEFAULT_ENDPOINT_SNIPPET_TEMPLATES: Record<
    FdrAPI.HttpMethod,
    APIV1Read.EndpointSnippetTemplates
  > = {
    PATCH: {
        ...
        if (result != null) {
          const value = {
            ...(toRet[result.endpointId.path] ??
              this.DEFAULT_ENDPOINT_SNIPPET_TEMPLATES),
