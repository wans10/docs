...
export class DocsDefinitionCacheImpl implements DocsDefinitionCache {
  ...
  public async storeDocsForUrl({
    ...
    await Promise.all(
      resp.domains.map(async (docsUrl) => {
        ...
        return await this.getDocsWriteMonitor(docsUrl.hostname).use(
          async () => {
  ...
  private async getDocsForUrlFromCache({
    ...
    url: URL;
  }): Promise<CachedDocsResponse | null> {
      ...
      return {
        ...
        response: {
          ...
          baseUrl: {
            ...
            basePath:
              dbDocs.path.trim() === "" ? undefined : dbDocs.path.trim(),
          },
          definition,
